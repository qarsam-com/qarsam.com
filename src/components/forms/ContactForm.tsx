...
export default function ContactForm() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(nextErrors).length === 0) {
      ...
      setSubmitted(false);
      alert("Thanks for reaching out. This demo form is ready for backend integration.");
    }
  };
    ...
    <form className="space-y-5" onSubmit={handleSubmit} noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <Input id="name" value={values.name} onChange={(event) => handleChange("name", event.target.value)} hasError={Boolean(errors.name)} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "name-error" : undefined} />
        ...
        <div>
          ...
          <Input id="company" value={values.company} onChange={(event) => handleChange("company", event.target.value)} />
        </div>
      </div>
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <Input id="email" type="email" value={values.email} onChange={(event) => handleChange("email", event.target.value)} hasError={Boolean(errors.email)} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "email-error" : undefined} />
        ...
        <div>
          ...
          <Input id="phone" value={values.phone} onChange={(event) => handleChange("phone", event.target.value)} hasError={Boolean(errors.phone)} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : undefined} />
      ...
      <div>
        ...
        <Select id="service" value={values.service} onChange={(event) => handleChange("service", event.target.value)} options={contactFormOptions} placeholder="Select a service" hasError={Boolean(errors.service)} aria-invalid={Boolean(errors.service)} aria-describedby={errors.service ? "service-error" : undefined} />
      ...
      <div>
        ...
        <Textarea id="projectDetails" value={values.projectDetails} onChange={(event) => handleChange("projectDetails", event.target.value)} hasError={Boolean(errors.projectDetails)} aria-invalid={Boolean(errors.projectDetails)} aria-describedby={errors.projectDetails ? "details-error" : undefined} />
